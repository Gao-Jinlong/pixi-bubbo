...
export class PhysicsSystem implements System {
  ...
  private _checkOutOfBounds(body: PhysicsBody) {
    ...
    // Check if the body has no more bounces and has gone below the bottom line
      ...
      body.bounces >= body.maxBounces &&
      body.y + body.radius > boardConfig.bubbleSize
    ) {
      // Kill the bubble
      bubble && level.killBubble(bubble);
    }
  ...
  private _checkShotConnect() {
    ...
    this._bodyMap.forEach((body) => {
      ...
      if (body === projectile.body || body.state !== PhysicsState.STATIC)
        return;
      ...
      if (distance < combinedRadius * 0.9) {
        ...
        if (connectedBubble) {
          level.handleConnect(projectile, {
            i: connectedBubble.i,
            j: connectedBubble.j,
          });
        }
