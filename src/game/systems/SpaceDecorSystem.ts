...
class BubbleOrbit {
  ...
  public changeView() {
    ...
    for (let i = 0; i < numOfSubBubbles; i++) {
      ...
      this._orbitRadii[i] =
        this._mainBubble.view.width * 0.5 +
        this._subBubbles[i].view.width * 0.5 +
        10;
      this._orbitSpeeds[i] =
        (this._subBubbles[i].view.scale.x / this._mainBubble.view.scale.x) *
        0.5;
  ...
  public update(delta: number) {
    ...
    for (let i = 0; i < this._subBubbles.length; i++) {
      ...
      this._subBubbles[i].view.x =
        Math.cos(this._orbitAngles[i]) * this._orbitRadii[i];
      this._subBubbles[i].view.y =
        Math.sin(this._orbitAngles[i]) * this._orbitRadii[i];

      // Update the orbit angle based on delta time and orbit speed
      this._orbitAngles[i] +=
        (delta / 60) * this._orbitSpeeds[i] * this._orbitDirections[i];
    }
  }
}
